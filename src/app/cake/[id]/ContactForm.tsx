...
export default function ContactForm({ cakeName }: { cakeName: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await fetch("/api/contact", {
        ...
        body: JSON.stringify({ ...form, cake_name: cakeName }),
      });
  ...
  const inputClass = "bg-white rounded-2xl px-5 py-4 text-[#201A1B] outline-none border border-[#F8BBD0]/30 focus:bg-[#FFF9C4]/50 focus:border-[#F8BBD0] transition-all placeholder:text-[#514346]/50 w-full";
